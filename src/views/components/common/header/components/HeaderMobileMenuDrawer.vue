...
interface BlizzardQuickLink {
  title: string;
  svg: string;
}

interface BlizzardSuggestion {
  title: string;
  type: string;
  badge: string;
  icon: string;
...
defineProps<{
  ...
  blizzardQuickLinks: BlizzardQuickLink[];
  blizzardSuggestions: BlizzardSuggestion[];
}>();
...
<template>
    <div class="lg:hidden fixed inset-0 z-[100000]">
      <button
        type="button"
      ...
      <aside
        class="relative h-full max-w-[390px] w-full bg-[#0f4a38] text-white overflow-y-auto shadow-[22px_0_50px_rgba(0,0,0,0.45)]"
      >
        <div class="p-3 border-b border-[#ffffff1a]">
          <div class="flex items-center justify-between mb-4">
            <button
              type="button"
              aria-label="Close menu"
              class="h-10 w-10 inline-flex items-center justify-center rounded-[8px] bg-[#3a6370] text-white cursor-pointer hover:brightness-125"
              @click="emit('close')"
            >
              <svg
            ...
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="#00aeff"
              aria-labelledby="blz-icon-title-blizzard"
              viewBox="0 0 128 64"
              part="icon"
              class="w-[71px] h-full hover:brightness-125 cursor-pointer"
            >
              <title id="blz-icon-title-blizzard">Blizzard</title>
              <path
                fill-rule="evenodd"
          ...
          <ul class="space-y-1">
            ...
            >
              <a
                href="#"
                class="block rounded-[8px] px-4 py-3 text-sm font-semibold hover:bg-[#ffffff14]"
                @click="emit('close')"
              >
                {{ item }}
              </a>
            ...
            <li>
              <a
                href="#"
                class="inline-flex items-center gap-2 px-4 py-3 text-sm font-semibold hover:bg-[#ffffff14] w-full rounded-[8px]"
                @click="emit('close')"
              >
                <span>Gear</span>
                <svg
                  ...
                  class="w-4 h-4"
                >
                  <path data-v-fe125f67="" d="M7 17 17 7m0 0H7m10 0v10"></path>
                </svg>
              </a>
            </li>
          </ul>
          ...
          <div class="mt-4 space-y-3">
            <button
              class="w-full min-h-[38px] bg-[#eb9110] hover:bg-[#f7b90a] rounded-[8px] text-black text-base cursor-pointer font-semibold"
            >
              Shop Upgrades
            </button>
            <button
              class="w-full min-h-[38px] bg-[#21ae7a] hover:bg-[#2cd196] rounded-[8px] text-black text-base cursor-pointer font-semibold"
            >
              Subscribe
            </button>
          </div>
        ...
        <div class="p-4 border-b border-[#ffffff10] bg-[#0a3b2d]">
          <p class="text-[#8bb9a8] text-xs font-bold tracking-wide mb-3">
            MORE FROM BLIZZARD
          </p>
          <ul class="space-y-1">
            <li
              v-for="(item, index) in blizzardQuickLinks"
              :key="`mobile-quick-${index}`"
            >
              <a
                href="#"
                class="flex items-center gap-3 rounded-[8px] px-2 py-2 hover:bg-[#ffffff10]"
              >
                <span
                  class="inline-flex h-6 w-6 items-center justify-center text-[#9ecfbf] [&_svg]:w-5 [&_svg]:h-5"
                  v-html="item.svg"
                ></span>
        ...
        <div class="p-4">
          ...
          <ul class="space-y-3">
            <li
              v-for="(item, index) in blizzardSuggestions"
              :key="`mobile-suggestion-${index}`"
            >
              <a
                href="#"
                class="flex items-center gap-3 rounded-[8px] px-1 py-1 hover:bg-[#ffffff10]"
              >
                <span
                  class="inline-flex h-11 w-11 items-center justify-center overflow-hidden rounded-full"
                ...
                <span class="min-w-0">
                  ...
                  <span class="flex items-center gap-2 mt-1">
                    <span class="text-[#b1c8bf] text-sm">{{ item.type }}</span>
                    <span
                      v-if="item.badge"
                      class="rounded px-2 py-[1px] text-[10px] font-bold tracking-wide text-black bg-[#f7b90a]"
                      >{{ item.badge }}</span
                    >
                  </span>
                </span>
